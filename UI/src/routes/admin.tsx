...
type AdminInfo = { id: number; email: string; role: string };
...
function AdminPage() {
  ...
  if (!admin) return <AuthGate onLogin={(info) => setAdmin(info)} />;
...
function AuthGate({ onLogin }: { onLogin: (info: AdminInfo) => void }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem("sastra_admin_token", access_token);
        ...
        <div className="rounded-3xl border border-primary/10 bg-card p-8 shadow-2xl">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            ...
            <button
              disabled={busy}
              type="submit"
              className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-bold text-primary-foreground disabled:opacity-60"
            >
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [b, m] = await Promise.all([
        ...
        apiGet<Message[]>("/api/admin/contacts"),
      ]);
      setBookings(b);
      setMessages(m);
    } catch {
      ...
      <header className="border-b border-primary/10 bg-background">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={load}
              className="rounded-lg border border-primary/15 p-2 hover:bg-secondary"
              title="Refresh"
            >
            ...
            <button
              onClick={signOut}
              className="inline-flex items-center gap-1.5 rounded-lg border border-primary/15 px-3 py-2 text-xs font-bold hover:bg-secondary"
            >
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-6 ${accent ? "border-primary/30 bg-primary text-primary-foreground" : "border-primary/10 bg-background"}`}
    >
...
function TabBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-full px-5 py-2 text-sm font-bold transition-colors ${active ? "bg-primary text-primary-foreground" : "text-foreground/70 hover:text-foreground"}`}
    >
      {children}
    </button>
  );
...
function Empty({ text }: { text: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-primary/20 bg-background p-16 text-center text-sm text-muted-foreground">
      {text}
    </div>
  );
}
  ...
  return (
    <article className="rounded-2xl border border-primary/10 bg-background p-6 shadow-sm">
      ...
      <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
        ...
        <Info icon={Calendar} label="Preferred date">
          {b.preferred_date ?? "—"}
        </Info>
        {b.lawn_size && (
          <Info icon={CheckCircle2} label="Lawn size">
            {b.lawn_size}
          </Info>
        )}
      </dl>
...
function MessageCard({
  ...
  return (
    <article className="rounded-2xl border border-primary/10 bg-background p-6 shadow-sm">
      ...
      <p className="mt-4 whitespace-pre-wrap rounded-lg bg-secondary/60 p-4 text-sm">
        {m.message}
      </p>
    </article>
  );
