...
app = FastAPI(title="S Astra LLC API", version="1.0.0")

origins = [
    "http://localhost:5173",
    "http://localhost:3000",
    "http://127.0.0.1:5173",
    "https://sastralawn.com",
    "https://www.sastralawn.com",
]
...
def _seed_admin():
    ...
    admin_email = os.getenv("ADMIN_EMAIL", "admin@sastralawn.com")
    db = SessionLocal()
    try:
...
def create_booking(data: BookingCreate, db: Session = Depends(get_db)):
    ...
    send_email(
        to_email=data.email,
        subject=f"Your S Astra Service Request {reference}",
...
def create_contact(data: ContactCreate, db: Session = Depends(get_db)):
    ...
    send_email(
        to_email=data.email,
        subject="Thank you for contacting S Astra",
...
def update_booking_status(
    ...
    send_email(
        ...
        subject=f"S Astra Service Update — {booking.reference}",
